...
const routes: Routes = [
  ...
  { path: 'edit-profile/:_id', loadChildren: () => import('./edit-profile/edit-profile.module').then(user => user.EditProfileModule), canActivate: [LoginGuard]},
  { path: 'change-password/:_id' , loadChildren: () => import('./change-password/change-password.module').then(user => user.ChangePasswordModule), canActivate: [LoginGuard] }
];
